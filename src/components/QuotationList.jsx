...
// 🚀 CRITICAL FIX 1: Import the context hook for dashboard refresh
import { useQuotationRefresh } from "../context/QuotationRefreshContext"; 

// Status Chip Component (Unchanged)
...
export default function QuotationList() {
    ...
    const navigate = useNavigate(); // ✅ This line is now resolved by the surrounding Router context

    // 🚀 CRITICAL FIX 2: Get the trigger function from the context
    const { triggerQuotationRefresh } = useQuotationRefresh(); 

    // Fetch quotations from API (Unchanged)
    React.useEffect(() => {
    ...
    // Get row ID (handles different field names) (Unchanged)
    const getRowId = (row) => row.quotationId || row.id;

    // Selection Handlers
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            // ✅ FIX 3: Use getRowId for robust ID tracking
            const newSelected = paginatedRows.map((n) => getRowId(n)); 
    ...
    // Generate PDF for a quotation (Unchanged)
    ...
    // View quotation - open PDF in new tab (Unchanged)
    ...
    // Edit quotation - navigate to edit form with quotation data (Unchanged)
    ...
    const handleChangeStatus = async (row, newStatus) => {
        try {
            ...
            // 🚀 CRITICAL FIX 3: Trigger the dashboard refresh after success
            ...
            >
                ...
                <Box sx={{ display: "flex", gap: 2 }}>
                    ...
                    <FormControl size="small">
                        {/* NOTE: This Select element currently does not apply filtering logic. */}
